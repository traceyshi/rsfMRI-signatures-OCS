...
#covariates
covariates_discovery <- data_discovery[data_discovery$matlab_include==1,c('interview_age','sex','site_id_l','mri_info_deviceserialnumber','race_ethnicity','rsfmri_c_ngd_meanmotion')]
...
#age (numeric)
covariates_discovery$interview_age <- as.numeric(as.character(covariates_discovery$interview_age))
covariates_discovery$scaled_age <- scale(covariates_discovery$interview_age) #standardize variable
mean(covariates_discovery$interview_age)
sd(covariates_discovery$interview_age)
class(covariates_discovery$scaled_age)

#sex (factor)
class(covariates_discovery$sex)
#site (factor)
class(covariates_discovery$site_id_l)
levels(covariates_discovery$site_id_l)
covariates_discovery$site_id_l <- droplevels(covariates_discovery$site_id_l) #drop unused levels
levels(covariates_discovery$site_id_l)
#scanner (factor)
class(covariates_discovery$mri_info_deviceserialnumber)
table(covariates_discovery$mri_info_deviceserialnumber)
covariates_discovery$mri_info_deviceserialnumber <- droplevels(covariates_discovery$mri_info_deviceserialnumber)
levels(covariates_discovery$mri_info_deviceserialnumber)
#race/ethnicity (factor)
class(covariates_discovery$race_ethnicity) #defaults to integer
covariates_discovery$race_ethnicity <- as.factor(covariates_discovery$race_ethnicity)
class(covariates_discovery$race_ethnicity)
levels(covariates_discovery$race_ethnicity)
table(covariates_discovery$race_ethnicity)
#only 2 participants coded as 888 -- remove these as causes issue with covariate regression from clinical features
covariates_discovery$race_ethnicity[covariates_discovery$race_ethnicity=='888'] <- NA
covariates_discovery$race_ethnicity <- droplevels(covariates_discovery$race_ethnicity)
missingraceeth <- which(is.na(covariates_discovery$race_ethnicity))
levels(covariates_discovery$race_ethnicity)

#mean motion (numeric)
covariates_discovery$rsfmri_c_ngd_meanmotion <- as.numeric(as.character(covariates_discovery$rsfmri_c_ngd_meanmotion))
covariates_discovery$scaled_motion <- scale(covariates_discovery$rsfmri_c_ngd_meanmotion) #standardize variable
mean(covariates_discovery$rsfmri_c_ngd_meanmotion, na.rm = TRUE)
sd(covariates_discovery$rsfmri_c_ngd_meanmotion, na.rm = TRUE)
class(covariates_discovery$scaled_motion)
...
cbcl_ocs_discovery <- cbcl_ocs_discovery[c(-missingraceeth),]
covariates_discovery <- covariates_discovery[c(-missingraceeth),]
connmats_raw_discovery <- connmats_raw_discovery[c(-missingraceeth),]
ids_discovery <- ids_discovery[c(-missingraceeth),]
ids_discovery <- ids_discovery$participant_id
subjectkey_discovery <- subjectkey_discovery[c(-missingraceeth),]
...
rm(data_abcd, data_discovery, matlab_subject_list_discovery, missingraceeth, workdir)
save.image("~/Desktop/OCS_bigdata_ML_local/FINAL/discovery/data/Data_prep/discovery_raw_data.RData")

#save discovery subject ID list
write.csv(subjectkey_discovery,file="~/Desktop/OCS_bigdata_ML_local/FINAL/discovery/data/Data_prep/subjectkey_discovery.csv",row.names=F)
```
...
for(i in 1:ncol(connmats_raw_discovery)) {
  model_i <- lm(connmats_raw_discovery[, i] ~ covariates_discovery$scaled_age + covariates_discovery$sex + covariates_discovery$mri_info_deviceserialnumber + covariates_discovery$race_ethnicity + covariates_discovery$scaled_motion, na.action = na.exclude)
...
#write.csv(rsFC_features_scaled, '/Users/marshlab2/Desktop/OCS_bigdata_ML_local/data/rsFC_features_scaled.csv', row.names = FALSE, col.names = FALSE)
...
# 
# ## generate covariate matrix for Winkler et al. CCA permutations
# # create dummy variables for all factor levels (leaving out one level for each factor variable)
# covariates_Z_discovery <- covariates_discovery[, c("scaled_age", "scaled_motion")]
# covariates_Z_discovery$sexF <- as.integer(covariates_discovery$sex == "F")
# covariates_Z_discovery$race_ethnicity2 <- as.integer(covariates_discovery$race_ethnicity == 2)
# covariates_Z_discovery$race_ethnicity3 <- as.integer(covariates_discovery$race_ethnicity == 3)
# covariates_Z_discovery$race_ethnicity4 <- as.integer(covariates_discovery$race_ethnicity == 4)
# covariates_Z_discovery$race_ethnicity5 <- as.integer(covariates_discovery$race_ethnicity == 5)
# 
# temp <- sapply(levels(covariates_discovery$mri_info_deviceserialnumber), function(x) as.integer(x == covariates_discovery$mri_info_deviceserialnumber))
# #drop level HASH03db707f for mri_info_deviceserialnumber
# covariates_Z_discovery <- cbind(covariates_Z_discovery, temp[,2:29])
# 
# write.csv(covariates_Z_discovery, '/Users/marshlab2/Desktop/OCS_bigdata_ML_local/data/covariates_Z_discovery.csv', row.names = FALSE, col.names = FALSE)
```
...
table(cbcl_ocs_discovery[,1], covariates_discovery$race_ethnicity)
...
clin_covariate_coeffs_count <- array(numeric(), c(7, ncol(cbcl_ocs_discovery))) #coefficients for poisson portion of model
clin_covariate_coeffs_zero <- array(numeric(), c(7, ncol(cbcl_ocs_discovery))) #coefficients for binary portion of model
clin_feat_normaltest <- array(numeric(), c(2, ncol(cbcl_ocs_discovery)))
  
for(i in 1:ncol(cbcl_ocs_discovery)) {
  model_i <- zeroinfl(cbcl_ocs_discovery[, i] ~ covariates_discovery$scaled_age + covariates_discovery$sex + covariates_discovery$race_ethnicity, dist = "poisson", na.action = na.exclude)
...
## Save raw clinical, rs-FC, and covariates data
```{r eval=FALSE, include=FALSE}
save.image("~/Desktop/OCS_bigdata_ML_local/FINAL/discovery/data/Data_prep/discovery_features_data.RData")
...
Store features as .csv for export to python (run this block as needed)
```{r}
write.csv(clin_features_scaled, '/Users/marshlab2/Desktop/OCS_bigdata_ML_local/FINAL/discovery/data/clin_features_scaled.csv', row.names = FALSE, col.names = FALSE)
write.csv(rsFC_features_scaled, '/Users/marshlab2/Desktop/OCS_bigdata_ML_local/FINAL/discovery/data/rsFC_features_scaled.csv', row.names = FALSE, col.names = FALSE)
...
#10-times 10-fold cross validation used for rsFC feature selection
...
write.csv(partitions2$Fold01.Rep1, '/Users/marshlab2/Desktop/OCS_bigdata_ML_local/FINAL/discovery/data/partitions2_rep01fold1.csv', row.names = FALSE, col.names = FALSE)
write.csv(partitions2$Fold02.Rep1, '/Users/marshlab2/Desktop/OCS_bigdata_ML_local/FINAL/discovery/data/partitions2_rep01fold2.csv', row.names = FALSE, col.names = FALSE)
write.csv(partitions2$Fold03.Rep1, '/Users/marshlab2/Desktop/OCS_bigdata_ML_local/FINAL/discovery/data/partitions2_rep01fold3.csv', row.names = FALSE, col.names = FALSE)
write.csv(partitions2$Fold04.Rep1, '/Users/marshlab2/Desktop/OCS_bigdata_ML_local/FINAL/discovery/data/partitions2_rep01fold4.csv', row.names = FALSE, col.names = FALSE)
write.csv(partitions2$Fold05.Rep1, '/Users/marshlab2/Desktop/OCS_bigdata_ML_local/FINAL/discovery/data/partitions2_rep01fold5.csv', row.names = FALSE, col.names = FALSE)
write.csv(partitions2$Fold06.Rep1, '/Users/marshlab2/Desktop/OCS_bigdata_ML_local/FINAL/discovery/data/partitions2_rep01fold6.csv', row.names = FALSE, col.names = FALSE)
write.csv(partitions2$Fold07.Rep1, '/Users/marshlab2/Desktop/OCS_bigdata_ML_local/FINAL/discovery/data/partitions2_rep01fold7.csv', row.names = FALSE, col.names = FALSE)
write.csv(partitions2$Fold08.Rep1, '/Users/marshlab2/Desktop/OCS_bigdata_ML_local/FINAL/discovery/data/partitions2_rep01fold8.csv', row.names = FALSE, col.names = FALSE)
write.csv(partitions2$Fold09.Rep1, '/Users/marshlab2/Desktop/OCS_bigdata_ML_local/FINAL/discovery/data/partitions2_rep01fold9.csv', row.names = FALSE, col.names = FALSE)
write.csv(partitions2$Fold10.Rep1, '/Users/marshlab2/Desktop/OCS_bigdata_ML_local/FINAL/discovery/data/partitions2_rep01fold10.csv', row.names = FALSE, col.names = FALSE)
...
```{r}
mean(covariates_discovery$interview_age)/12
sd(covariates_discovery$interview_age)/12
range(covariates_discovery$interview_age)/12

table(covariates_discovery$sex)[["F"]]/length(covariates_discovery$sex)

#CBCL-OCS total score
t <- rowSums(cbcl_ocs_discovery)
mean(t)
sd(t)
...
table(cbcl_ocs_discovery_binary[,1])[["TRUE"]]/length(cbcl_ocs_discovery_binary[,1])
table(cbcl_ocs_discovery_binary[,2])[["TRUE"]]/length(cbcl_ocs_discovery_binary[,1])
table(cbcl_ocs_discovery_binary[,3])[["TRUE"]]/length(cbcl_ocs_discovery_binary[,1])
table(cbcl_ocs_discovery_binary[,4])[["TRUE"]]/length(cbcl_ocs_discovery_binary[,1])
table(cbcl_ocs_discovery_binary[,5])[["TRUE"]]/length(cbcl_ocs_discovery_binary[,1])
table(cbcl_ocs_discovery_binary[,6])[["TRUE"]]/length(cbcl_ocs_discovery_binary[,1])
table(cbcl_ocs_discovery_binary[,7])[["TRUE"]]/length(cbcl_ocs_discovery_binary[,1])
table(cbcl_ocs_discovery_binary[,8])[["TRUE"]]/length(cbcl_ocs_discovery_binary[,1])

#temp
tc <- array(numeric(),c(8,2))
tc[1,1] <- table(cbcl_ocs_discovery_binary[,1])[["TRUE"]]/length(cbcl_ocs_discovery_binary[,1])
tc[2,1] <- table(cbcl_ocs_discovery_binary[,2])[["TRUE"]]/length(cbcl_ocs_discovery_binary[,1])
tc[3,1] <- table(cbcl_ocs_discovery_binary[,3])[["TRUE"]]/length(cbcl_ocs_discovery_binary[,1])
tc[4,1] <- table(cbcl_ocs_discovery_binary[,4])[["TRUE"]]/length(cbcl_ocs_discovery_binary[,1])
tc[5,1] <- table(cbcl_ocs_discovery_binary[,5])[["TRUE"]]/length(cbcl_ocs_discovery_binary[,1])
tc[6,1] <- table(cbcl_ocs_discovery_binary[,6])[["TRUE"]]/length(cbcl_ocs_discovery_binary[,1])
tc[7,1] <- table(cbcl_ocs_discovery_binary[,7])[["TRUE"]]/length(cbcl_ocs_discovery_binary[,1])
tc[8,1] <- table(cbcl_ocs_discovery_binary[,8])[["TRUE"]]/length(cbcl_ocs_discovery_binary[,1])

tc[1,2] <- 0.703837414838958
tc[2,2] <- 0.774621931533237
tc[3,2] <- 0.671796189220894
tc[4,2] <- 0.789480976522737
tc[5,2] <- 0.841248475500628
tc[6,2] <- 0.884250117632865
tc[7,2] <- 0.85561488431253
tc[8,2] <- 0.682450688734507

plot(tc, xlab = "Percentage of participants with symptom X", ylab = "AUROC for model predicting symptom X")
```
